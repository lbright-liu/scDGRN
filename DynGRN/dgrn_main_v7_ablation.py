...
from gRNN_v7_ablation import GRNN
...
d_type = 'mHSC-GM'
#d_type_2 = 'sim2'
d_short = 'mGMc3_expression'
#d_short = 'mhepc5_expression'
...
## 最新的维度：h1 = 100 h2 = 64 m1 =250 m2 = 100
## 最最新的维度：h1 = 64 h2 = 64 m1 =250 m2 = 100
## h1 = 64 h2 =85  m1 = 150 m2 = 100 mEc3 = 250 mGMc3 = 128 mLc2 = 200
## hesc2因果推断的维度：D=32更好
model = GRNN(input_dim=128, # 这个输入维度是什么操作，细胞的个数,正确的
...
if args.flag:
    ...
    # plt.plot(fpr, tpr, color='red', linewidth=6)
    # plt.grid()
    ...
    # auc = round(auc, 3)
    # plt.title('DynGRN:' + str(auc), fontsize=20)
    # plt.savefig('./draw' + '/overall.png', dpi=600, bbox_inches='tight')

    # ## 计算假阳性比例
    # b_bst = 100
    # b_index = -1
    # import math
    # fpr, tpr, thresholds = metrics.roc_curve(test_data[:, -1].cpu().detach().numpy(), score.cpu().detach().numpy(), pos_label=1)
    # for i in range(len(thresholds)):
    #     d = math.sqrt((fpr[i] - 0) ** 2 + (tpr[i] - 1) ** 2)
    #     #m_max.append(d)
    #     if d < b_bst:
    #         b_bst = d
    #         b_index = i
    #
    # b_thred = thresholds[b_index]
    # # 统计假阳性样本
